...
from trading_metrics import load_trading_data, calculate_trading_metrics
...
def load_data():
    ...
    if Path("output/trading_data_20250608.csv").exists():
        df_trades = pd.read_csv("output/trading_data_20250608.csv")
        asset_metrics = pd.read_csv("output/asset_trading_metrics_20250608.csv")
...
# Main app
def main():
    st.title("⚡ FlexPower Trading Metrics Dashboard")
    st.markdown("### Task 3: Trading Performance for June 8, 2025")
    st.markdown("---")
